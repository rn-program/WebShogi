...
// 選択中の駒と選択中の持ち駒、初期局面
let board_sfen = "lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL b - 1";
                ...
                if (selectedPiece) {
                    ...
                    if (canPromote) {
                        ...
                        socket.on("legal_move", () => {
                            ...
                            promoteBtn.onclick = () => {
                                socket.emit("get_move", { move: moveStrPromote, board_sfen: board_sfen });
                                selectedPiece = null;
                            };
                            noPromoteBtn.onclick = () => {
                                socket.emit("get_move", { move: moveStrNormal, board_sfen: board_sfen });
                                selectedPiece = null;
                            };
                        });
                    } else { // 駒が成れないときの処理
                        const moveStr = coordsToSfenMove(moveFrom.row, moveFrom.col, moveTo.row, moveTo.col);
                        socket.emit("get_move", { move: moveStr, board_sfen: board_sfen });
                        selectedPiece = null;
                    }
                }
            });
